...
<html>
          ...
          <input type="checkbox" id="nav-toggle-cb" checked="checked">
            <nav id="menu">
              ...
              <div class="input-group" id="preferences-group">
                ...
                <button id="sleep" title="Sleep the camera module">Sleep</button>
  ...
  document.addEventListener('DOMContentLoaded', function (event) {
    ...
    sleepButton.onclick = () => {
      if (confirm("Sleep the Camera Module?")) {
        updateConfig(sleepButton);
        // Some sort of countdown here?
        hide(settings);
        hide(viewContainer);
        header.innerHTML = '<h1>Sleeping!</h1><hr>Page will reload after 30 seconds.';
        setTimeout(function() {
          location.replace(document.URL);
        }, 30000);
      }
